...
import { fmt, fmtExact, parseAmount, addMonths, monthLabel, h, raw } from '../util.js';
...
function splitControl() {
  ...
  return h`<div class="split-control">
    ...
    ${splitPreset === 'custom' ? h`<div class="custom-split-panel">
      ...
      ${!valid ? raw('<span class="split-error">Adjust the three amounts so the total is 100%.</span>') : ''}
    </div>` : ''}
  </div>`;
}
...
function progressBar(actual, target, cls) {
  ...
  return h`<div class="class-progress">
    <div class="class-progress-track">
      ...
      ${over > 0 ? h`<div class="class-progress-over" style="width:${Math.min(100, over)}%"></div>` : ''}
    </div>
  </div>`;
}
...
function donutSvg(data) {
  ...
  if (!total) {
    return h`<svg viewBox="0 0 180 180" class="fifty-donut-svg">
  ...
  const arcs = CLASS_ORDER.map(cls => {
    ...
    acc += frac;
    return h`<circle cx="${CX}" cy="${CY}" r="${R}" fill="none" stroke="var(--fifty-${cls})"
...
function comparisonSection(data, split) {
  return h`<section class="fifty-section">
    ...
    <div class="class-cards">
      ${CLASS_ORDER.map(cls => classCard(data, split, cls))}
    </div>
    <div class="donut-row">
...
function classManager(data) {
  ...
  return h`<section class="fifty-section">
    ...
    ${groups.map(g => h`<div class="class-group">
      ...
      ${!g.rows.length ? raw('<p class="muted class-group-empty">Nothing here yet.</p>') : h`<div class="class-rows">
        ${g.rows.map(r => h`<div class="class-row">
          ...
          <div class="class-row-picker">
            ...
            ${openClassMenuId === r.id ? h`<div class="class-row-menu" role="listbox" aria-label="Classify ${r.name}">
              ${CLASS_ORDER.map(cls => h`<button role="option" aria-selected="${r.cls === cls}" class="${r.cls === cls ? 'selected' : ''}" data-act="set-class" data-id="${r.id}" data-class="${cls}"><span class="class-dot dot-${cls}"></span><span>${cls === 'need' ? 'Need' : cls === 'want' ? 'Want' : 'Savings'}</span>${r.cls === cls ? raw('<b>✓</b>') : ''}</button>`)}
            </div>` : ''}
          </div>
        </div>`)}
      </div>`}
